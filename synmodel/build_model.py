...
'''
# ---- Refine basic grid (focus on external buffer management)
for ref_id, path in shp_dic.items():
        if 'refine' in ref_id:
                ftype = 'polygon'
                level = ref_id.split('_')[1]
                g.add_refinement_features(path.replace('.shp',''), ftype, level, range(nlay))
'''
...
'''
# ---- Plot base model discretisation (refine)
fig = plt.figure(figsize=(15, 15))
ax = fig.add_subplot(1, 1, 1, aspect='equal')
...
for ref_id, data in refine_dic.items():
        ...
        if ftype == 'line':
                gdf.plot(ax=ax, lw=1)
        if ftype == 'point':
                gdf.plot(ax=ax, color='r', markersize=5)
plt.show()
'''


# ref1 = gpd.read_file(os.path.join(sig_dir,'refine_1.shp'))
# shp = gpd.GeoDataFrame({'FID':['buffer'], 'level':[1]}, geometry=[ref1.geometry.unary_union])
# shp.to_file(os.path.join(sig_dir, 'refine_1.shp'))
# ref2 = gpd.read_file(os.path.join(sig_dir,'refine_2.shp'))
# shp = gpd.GeoDataFrame({'FID':['buffer'], 'level':[2]}, geometry=[ref2.geometry.unary_union])
# shp.to_file(os.path.join(sig_dir, 'refine_2.shp'))


'''
# # ---- Export base modelgrid
# b_dis.export(os.path.join(sig_dir, 'base_modelgrid.shp'))
# # ---- Build river
# A = Point((ncol*delc)/2, nrow*delr)
# B = Point(ncol*delc, (nrow*delr) - (nrow*delr)/10)
# C = Point((ncol*delc)/2, (nrow*delr) - (nrow*delr)/2)
# D = Point((ncol*delc)/2, 0)
# reach_1 = LineString([A,C])
# reach_2 = LineString([B,C])
# reach_3 = LineString([C,D])
# river   = MultiLineString([reach_1, reach_2, reach_3])
# river_pts = gpd.GeoDataFrame({'FID':list('ABCD')}, geometry=[A,B,C,D])
# river = gpd.GeoDataFrame({'FID':[f'reach_{i}' for i in range(3)]}, geometry=[reach_1, reach_2, reach_3])

# chd_line = LineString([Point(0,0),Point(ncol*delc, 0)])
# chd = gpd.GeoDataFrame({'FID':['chd']}, geometry=[chd_line])
# chd.to_file(os.path.join(sig_dir,'chd.shp'))


# well_1 = Point(2525,2725)
# well_2 = Point(1225,1325)
# wells = gpd.GeoDataFrame({'FID':['well_1','well_2']}, geometry=[well_1,well_2])
# wells.to_file(os.path.join(sig_dir,'wells.shp'))

# drain_line = LineString([Point(2075,4075), Point(2075,4075 - 8*delr)])
# drain = gpd.GeoDataFrame({'FID':['drain_1']}, geometry=[drain_line])
# drain.to_file(os.path.join(sig_dir,'drain.shp'))
'''
...
# -- Fetch raster of heterogenous hydraulic conditivity 
#    generated by USGS (Unconditional Sequence Gaussian Simulation)
#    with an exponential variogram (psill=0.3, range=500, nugget=0)

k_ras = flopy.utils.Raster.load(os.path.join(sig_dir, 'k.tiff'))
...
q_well_dic = {k:v for k,v in zip(icpl_dic['wells'].keys(), [100,20])}  # pumping rates in m3/h
...
# ---- Plot head data
heads = pmv.plot_array(hds, masked_values=[1.e+30], cmap ='viridis', alpha=0.5)
cb = plt.colorbar(heads, shrink = 0.5)

# ---- Plot contours
hmin, hmax = hds.min(), hds.max()
levels = np.arange(10, np.ceil(hmax), 0.75)
